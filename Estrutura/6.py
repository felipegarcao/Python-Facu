    ...
    if tipo_servico == []:
        ...
        while desejaCadastrar == True:
            ...
            if len(servicos[dia-1]) < 3:
                ...
                servico = Servico()
                servico.numero_servico = int(
                    input("Digite o número do serviço: "))
                servico.valor_servico = float(
                    input("Digite o valor do serviço: "))
                servico.codigo_servico = int(
                    input("Digite o código do tipo de serviço: "))
                servico.codigo_cliente = int(
                    input("Digite o código do cliente: "))
            ...
            resposta = input(
                "Deseja cadastrar mais um serviço prestado? (S/N): ")
            if resposta.lower() == "s":
                desejaCadastrar = True
            else:
    ...
    for dia in servicos:
        ...
        if dia != []:
            ...
            for servico in dia:
                ...
                dados.append([contador, str(servico.numero_servico), str(servico.valor_servico), str(
                    servico.codigo_servico), pegaTipo(servico.codigo_servico), str(servico.codigo_cliente)])

    print(tabulate(dados, headers=["Dia", "No de serviço", "Valor do serviço R$",
          "Código de serviço", "Descrição", "Código de cliente"]))
    menu()
    ...
    for dia in servicos:
        ...
        if dia != []:
            ...
            for servico in dia:
                ...
                dados.append([str(servico.numero_servico), str(servico.valor_servico), str(
                    servico.codigo_servico), pegaTipo(servico.codigo_servico), str(servico.codigo_cliente)])

            print(tabulate(dados, headers=[
                  "No de serviço", "Valor do serviço R$", "Código de serviço", "Descrição", "Código de cliente"]))
        else:
...
def consulta_dia():
    ...
    print(tabulate(dados, headers=["No de serviço", "Valor do serviço R$",
          "Código de serviço", "Descrição", "Código de cliente"]))
